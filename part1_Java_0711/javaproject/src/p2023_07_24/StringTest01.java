...
//  toLowerCase(); 소문자로 변환 해주는 역할
	...
	// 변경된 값을 heap 메모리 영역에 다시 저장한다.
	// 2. heap메모리 영역에 변경된 값을 재사용 하기 위해서는 새로운 변수로 변경된 값을 저장해서 사용
	// ex) String str2 = str1.toUpperCase();
	// 3. heap메모리 영역에 변경된 값을 재사용할 수 없을 경우에는 garbage로 인식하고
	// heap메모리 영역의 데이터를 garbage collector program이 모아서 지워버린다.
	// ex) str1.toUpperCase();
